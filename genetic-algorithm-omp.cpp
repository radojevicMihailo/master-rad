// Genetic Algorithm — OpenMP-parallel C++ version
// Parallelizes: population init, fitness evaluation, child generation.
// Identical algorithm/results-schema to genetic-algorithm.cpp, just faster on
// many-core machines (the whole point for big populations on AWS).
//
// Build (Linux / AWS, real gcc):
//   g++ -O3 -march=native -flto -fopenmp -std=c++17 genetic-algorithm-omp.cpp -o ga-omp
// Build (macOS, Apple clang needs libomp):
//   brew install libomp
//   clang++ -O3 -std=c++17 -Xpreprocessor -fopenmp \
//     -I$(brew --prefix libomp)/include -L$(brew --prefix libomp)/lib -lomp \
//     genetic-algorithm-omp.cpp -o ga-omp
// Run:
//   OMP_NUM_THREADS=<cores> ./ga-omp     (defaults to all cores)
...
// int16_t: cell values bounded by min(DEMAND, MAX_UNITS); for current datasets
// max units = 720 (10x10) / 576 (1000x1000) << 32767, so int16 is safe (int8 is NOT).
using Gene = int16_t;
...
// ── RNG (thread-safe) ────────────────────────────────────────────────────────
// One generator per thread, distinctly seeded. Global mt19937 is NOT thread-safe,
// so every parallel region must go through these.
static std::mt19937 &thread_rng()
{
    static std::random_device seed_source;
    thread_local std::mt19937 generator(seed_source() ^
...
inline int randint(int low, int high) { return std::uniform_int_distribution<int>(low, high)(thread_rng()); }
inline double randreal() { return std::uniform_real_distribution<double>(0.0, 1.0)(thread_rng()); }
...
#pragma omp parallel for schedule(dynamic)
        ...
        // Children fill fixed slots so iterations are independent (parallel-safe).
        int num_children = POPULATION_SIZE - ELITE_COUNT;
        int num_pairs = (num_children + 1) / 2;
#pragma omp parallel for schedule(dynamic)
        for (int p = 0; p < num_pairs; ++p)
        {
            ...
            mutate(child1);
            mutate(child2);
            int slot = ELITE_COUNT + 2 * p;
            new_population[slot] = std::move(child1);
            if (slot + 1 < POPULATION_SIZE)
                new_population[slot + 1] = std::move(child2);
        }
...
static void save_results(const std::vector<RunResult> &runs)
{
    ...
    name << "ga-cpp-omp-" << POPULATION_SIZE << "-" << GENERATIONS << "-"
         << ELITE_COUNT << ".txt";
    fs::path filepath = txt_dir / name.str();

    std::ofstream file(filepath);
    file << "GA Hiperparametri (C++ OpenMP):\n"
    ...
    csv_name << "ga-cpp-omp-convergence-" << POPULATION_SIZE << "-" << GENERATIONS << "-"
             << ELITE_COUNT << ".csv";
...
#ifdef _OPENMP
    std::cout << "OpenMP niti: " << omp_get_max_threads() << "\n";
#else
    std::cout << "UPOZORENJE: kompajlirano BEZ OpenMP (serijski).\n";
#endif
